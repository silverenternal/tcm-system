...
本项目采用多层次的安全机制防止SQL注入攻击：

1. **Spring Data JPA参数化查询** - 所有数据库操作都通过JPA Repository，使用参数绑定而非字符串拼接
2. **方法命名查询** - 使用Spring Data JPA的方法命名约定自动生成安全的查询
3. **JPQL + @Param注解** - 自定义查询使用参数化的JPQL，确保输入与SQL结构分离
