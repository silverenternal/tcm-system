...
const SelfDiagnosis = {
  ...
  methods: {
    ...
    submitImage() {
      ...
      // 上传舌象图片并触发AI分析
      // 删除ID卡号字段，让后端处理为null
      delete this.diagnosisData.patient.idCard;
    ...
    saveDiagnosisData() {
      // 为自诊患者不设置ID卡号（完全移除字段），让后端处理为null
      delete this.diagnosisData.patient.idCard;
          ...
          return ApiService.post('/api/visits', visitData)
            .then(visitResponse => {
              ...
              return ApiService.post(`/api/self-diagnosis/complete-self-diagnosis/${visit.id}`)
                .then(aiResponse => {
                  this.addSystemMessage("您的自诊信息已保存成功！AI分析将在后台进行。");

                  // 显示AI分析提交状态，由于是异步处理，暂时无法立即获得结果
                  if (aiResponse.data && aiResponse.data.status === "submitted") {
                    this.addSystemMessage("AI分析已在后台启动，请稍后查看结果。");

                    // 我们可以使用轮询来检查结果是否准备好
                    this.pollForAIResult(visit.id);
                  ...
                  console.log('诊断数据保存成功', { patient, visit, aiResponse: aiResponse.data });
                })
                .catch(aiError => {
    ...
    pollForAIResult(visitId) {
      ...
      const poll = () => {
        ...
        ApiService.get(`/api/self-diagnosis/analysis-result/${visitId}`)
          .then(response => {
            if (response.data && response.data.result) {
              ...
              if (attempts < maxAttempts) {
                ...
                this.addSystemMessage("AI分析可能需要更长时间，请稍后再查看结果。");
              }
            }
          })
          .catch(error => {
            ...
            if (attempts < maxAttempts) {
              setTimeout(poll, 2000); // 出错时也继续轮询
            }
          });
      };

      // 开始轮询
      setTimeout(poll, 2000); // 首次延迟2秒开始轮询
    }
  }
};
